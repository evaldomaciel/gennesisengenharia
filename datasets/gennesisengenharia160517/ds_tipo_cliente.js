function createDataset(fields, constraints, sortFields) {
    ...
    if (constraints != null && constraints.length && constraints[0].fieldName != "sqlLimit") { //se tiver constraint filtra
        for (var index = 0; index < constraints.length; index++) {
            tipo = constraints[index].fieldName == "Tipo" ? constraints[0].initialValue : tipo; // Se o ID for informado, é valor é atribuído a variável
...
function getDefaultValues() {
    return [
        ['Tipo A','NOME DO CLIENTE A'],
        ['Tipo B','NOME DO CLIENTE B']
    ]    
}
