function createDataset(fields, constraints, sortFields) {
    ...
    if (constraints != null) {
        for (var a = 0; a < constraints.length; a++) {
            ...
            if (constraints[a].fieldName == 'CODCOLIGADA') {
                coligada = constraints[a].initialValue;
            } 
        }
    }
   
    var filter = '?context=CODCOLIGADA%3D1&parameters=CODCOLIGADA%3D' + coligada + '%3B'
    try {
        ...
        var data = {
            ...
            serviceCode: 'RestRM',
            endpoint: '/ZZZ.CENTROCUSTO/0/M' + filter,
            method: 'GET',
        ...
        if (status == 200 || status == 201) {
            ...
            if (ret.length > 0) {
                ...
                dataset.addColumn("pesquisar_campos");
                ...
                for (var i = 0; i < ret.length; i++) {
                    ...
                    var campos = ret[i][cols[2]] + ", " + ret[i][cols[3]];
                    dataArr.push(campos);
                    for (var c = 0; c < cols.length; c++) {
