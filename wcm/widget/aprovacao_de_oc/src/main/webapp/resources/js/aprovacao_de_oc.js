...
var ExportaProcesso = SuperWidget.extend({
    ...
    mudaStatusAPR: function (params) {
        switch (params) {
            case "0" || 0: return "Em andamento";
            case "2" || 2: return "Aprovado";
            default: return "Não aprovado";
        }
    },
    ...
    mudaCorStatusG4: function (params) {
        switch (params) {
            case "Aberta" || 0: return "text-success";
            case "Cancelada" || 1: return "text-warning";
            case "Finalizada" || 2: return "text-danger";
            default: return "Desconhecido";
        }
    },
    ...
    loadTable: function (htmlElement, event) {
        ...
        setTimeout(() => {
            mydata = DatasetFactory.getDataset("dsWidgetAprOC", null, constraints, null);

            mydata['values'].forEach(element => {
                loading.show();
                element['CORSTATUS'] = this.mudaCorStatus(element['STATUS']);
                element['CORSTATUSG4'] = this.mudaCorStatus(element['G4_STATUS']);
                element['STATUS'] = this.mudaStatus(element['STATUS']);
                element['G4_STATUS'] = this.mudaStatus(element['G4_STATUS']);
                element['STATUSAPR'] = this.mudaStatusAPR(element['STATUS']);
                dateExcel.push({
                    "Status G3": element['DSG3StatusFluig'],
                    "Status G4": element['G4_STATUS'],
                    ...
                    "Data de Receimento": element['dataRecebimen'],
